...
public class AddUploadFileAction implements IAction {
	private static Logger logger = Logger.getLogger(AddUploadFileAction.class);
	private IAgileSession session;
	private IDocumentDTO doc;

	public AddUploadFileAction(IDocumentDTO doc) {
		this.doc = doc;
	}
	...
	public Integer doAction() {
		...
		try {
				...
				if (doc.getFiles() != null) {
					...
					for(int i=0;i<files.length;i++){
						...
						if(value==null){//不存在
							...
							HazyUtil.getAgileAPIHelper().uploadAddFile(session, item, files[i]);
						}
					}
			...
			doc.getDataObject().setValue(title.getIndex(TitleConstants.AddFileStatus), "FAILURE");
			return FAILURE;
		} finally {
			if (session != null) {
				session.close();
		...
		if (resultStr.length() == 0) {
			...
			doc.getDataObject().setValue(title.getIndex(TitleConstants.AddFileStatus), "SUCCESS");// Validation结果
		} else {
			doc.setStatus(FAILURE);
			doc.getDataObject().setValue(title.getIndex(TitleConstants.AddFileStatus), resultStr.toString());// Validation结果
		}
